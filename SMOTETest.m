...
[SMOTE_featuresSyn] = SMOTE(features0, 19, 25);
...
figure;
hold on;
plot(SMOTE_featuresSyn(:,1), SMOTE_featuresSyn(:,2), 'g.');
plot(features0(:,1), features0(:,2), 'r.');
plot(features1(:,1), features1(:,2), 'b.');
title('input point sets and synthetic points generated bySMOTE');
legend({['synthetic minority class (' num2str(size(SMOTE_featuresSyn,1)) ' examples)'] ['minority class (' num2str(numEx0) ' examples)'] ['majority class (' num2str(numEx1) ' examples)']});
xlabel('feature_1');
ylabel('feature_2');
axis('equal');
hold off;
